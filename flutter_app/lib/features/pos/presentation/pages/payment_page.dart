...
class _PaymentPageState extends ConsumerState<PaymentPage> {
        ...
        return AlertDialog(
          ...
          actions: [
            TextButton(
              ...
              child: const Text('Print Options'),
            ),
            TextButton(
              onPressed: () async {
                try {
                  ...
                  final bytes = await InvoicePdfBuilder.buildPdfFromHtml(sale, company, format: PdfPageFormat.a4);
  ...
  Future<void> _showPrintOptions(PosCheckoutResult result) async {
    ...
    final hasThermal = settings != null && settings.connectionType == 'network' && (settings.host?.isNotEmpty ?? false) && (settings.port ?? 0) > 0;
      ...
      builder: (ctx) {
        return SafeArea(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              ListTile(title: const Text('Select Print Option'), dense: true),
              if (hasThermal) ...[
                ListTile(
                  leading: const Icon(Icons.print_rounded),
                  title: const Text('Thermal 80mm'),
                  onTap: () async {
                    Navigator.of(ctx).pop();
                    try {
                      await printThermalOverTcp(sale: sale, company: company, settings: settings!, paperSize: '80mm');
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Printed to thermal 80mm')));
                      }
                    } catch (e) {
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Thermal print failed: $e')));
                      }
                    }
                  },
                ),
                ListTile(
                  leading: const Icon(Icons.print_rounded),
                  title: const Text('Thermal 58mm'),
                  onTap: () async {
                    Navigator.of(ctx).pop();
                    try {
                      await printThermalOverTcp(sale: sale, company: company, settings: settings!, paperSize: '58mm');
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Printed to thermal 58mm')));
                      }
                    } catch (e) {
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Thermal print failed: $e')));
              ...
              ListTile(
                ...
                onTap: () async {
                  ...
                  await Printing.layoutPdf(
                    onLayout: (format) => InvoicePdfBuilder.buildPdfFromWidgets(sale, company, format: PdfPageFormat.a5),
                  );
                },
              ),
              ListTile(
                ...
                onTap: () async {
                  Navigator.of(ctx).pop();
                  await Printing.layoutPdf(
                    onLayout: (format) => InvoicePdfBuilder.buildPdfFromWidgets(sale, company, format: PdfPageFormat.a4),
                  );
                },
              ),
            ],
