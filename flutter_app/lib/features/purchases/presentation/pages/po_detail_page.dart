...
class _PoDetailPageState extends ConsumerState<PoDetailPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final canApprove = !_loading &&
        (status != 'APPROVED' &&
            status != 'PARTIALLY_RECEIVED' &&
            status != 'RECEIVED');
    final canReceive = !_loading &&
        hasRemaining &&
  ...
  Future<void> _receive() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context)
        ..hideCurrentSnackBar()
        ..showSnackBar(SnackBar(content: Text('Receive failed: $e')));
    }
  }
}
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(po['purchase_number']?.toString() ?? '',
...
class _ReceiveDialog extends StatefulWidget {
  ...
  final List<Map<String, dynamic>>
      lines; // {purchase_detail_id, product_name, remaining}
  ...
  void initState() {
    ...
    _qty = widget.lines
        .map((e) => TextEditingController(
            text: (e['remaining'] as double).toStringAsFixed(2)))
        .toList();
  }
            ...
            onPressed: () {
              ...
              for (int i = 0; i < widget.lines.length; i++) {
                ...
                if (qty > 0) {
                  items.add({
                    'purchase_detail_id':
                        widget.lines[i]['purchase_detail_id'] as int,
