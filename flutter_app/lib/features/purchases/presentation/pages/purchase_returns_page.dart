...
class PurchaseReturnsPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<PurchaseReturnsPage> createState() => _PurchaseReturnsPageState();
}
...
class _PurchaseReturnsPageState extends ConsumerState<PurchaseReturnsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final filtered = q.isEmpty
        ? _all
        : _all.where((e) => (e['return_number'] ?? '').toString().toLowerCase().contains(q)).toList();
    return Scaffold(
      appBar: AppBar(
                        ...
                        itemBuilder: (context, i) {
                          ...
                          return Card(
                            ...
                            child: ListTile(
                              ...
                              title: Text(pr['return_number']?.toString() ?? ''),
                              subtitle: Text([
                                if ((pr['supplier']?['name'] ?? pr['supplier_name'] ?? '') != '') (pr['supplier']?['name'] ?? pr['supplier_name']).toString(),
                                if (pr['return_date'] != null) pr['return_date'].toString(),
                                if ((pr['purchase']?['purchase_number'] ?? '') != '') 'From: ${pr['purchase']['purchase_number']}',
...
class _ReturnFormPageState extends ConsumerState<_ReturnFormPage> {
  ...
  Future<void> _linkPurchaseForSupplier() async {
    ...
    setState(() { _loadingLink = true; _linkedPurchase = null; _linkedPurchaseId = null; });
    try {
      final repo = ref.read(purchasesRepositoryProvider);
      final listRec = await repo.getOrders(status: 'RECEIVED', supplierId: supplierId);
      final listPar = await repo.getOrders(status: 'PARTIALLY_RECEIVED', supplierId: supplierId);
      final list = [...listRec, ...listPar];
      if (list.isEmpty) return;
      // pick the latest by purchase_date
      list.sort((a,b){
        final da = DateTime.tryParse((a['purchase_date'] ?? '').toString()) ?? DateTime.fromMillisecondsSinceEpoch(0);
        final db = DateTime.tryParse((b['purchase_date'] ?? '').toString()) ?? DateTime.fromMillisecondsSinceEpoch(0);
      ...
      if (id != null) {
        ...
        if (!mounted) return;
        setState(() { _linkedPurchaseId = id; _linkedPurchase = po; });
      }
    } finally {
  ...
  Future<void> _save() async {
    final supplierId = _supplierId;
    if (supplierId == null) {
      ScaffoldMessenger.of(context)..hideCurrentSnackBar()..showSnackBar(const SnackBar(content: Text('Select supplier')));
      return;
    }
    // Build lines payload
    ...
    if (payload.isEmpty) {
      ScaffoldMessenger.of(context)..hideCurrentSnackBar()..showSnackBar(const SnackBar(content: Text('Enter quantities to return')));
      return;
    }
    try {
      ...
      if (purchaseId == null || purchase == null) {
        ScaffoldMessenger.of(context)..hideCurrentSnackBar()..showSnackBar(const SnackBar(content: Text('No existing purchase found for supplier to link return')));
        return;
      }
      // map purchase_detail_id if available
      final details = (purchase['items'] as List? ?? const []).cast<Map<String, dynamic>>();
      ...
      if (file.isNotEmpty) {
        try { await ref.read(purchaseReturnsRepositoryProvider).uploadReceipt(returnId: id, filePath: file, receiptNumber: number.isEmpty ? null : number); } catch (_) {}
      }
      if (!mounted) return;
          ...
          children: [
            _SupplierPicker(
              ...
              onPicked: (id, name) async {
                setState(() { _supplierId = id; _supplierName = name; });
                await _linkPurchaseForSupplier();
              },
            ),
            ...
            ..._buildLines(context),
            const SizedBox(height: 8),
            Align(alignment: Alignment.centerLeft, child: OutlinedButton.icon(onPressed: () => setState(() => _lines.add(_RetLine())), icon: const Icon(Icons.add_rounded), label: const Text('Add Item'))),
        ...
        Card(
          ...
          child: Padding(
            ...
            child: Column(children: [
              ...
              const SizedBox(height: 8),
              Row(children: [
                Expanded(child: TextField(controller: _lines[i].qty, keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: const InputDecoration(labelText: 'Quantity', prefixIcon: Icon(Icons.format_list_numbered_rounded)))),
                const SizedBox(width: 8),
                Expanded(child: TextField(controller: _lines[i].price, keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: const InputDecoration(labelText: 'Unit Price', prefixIcon: Icon(Icons.currency_rupee_rounded)))),
                IconButton(onPressed: _lines.length == 1 ? null : () => setState(() => _lines.removeAt(i)), icon: const Icon(Icons.delete_outline_rounded))
              ]),
            ]),
          ),
        ),
...
class _LineProductPickerState extends ConsumerState<_LineProductPicker> {
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      ...
      child: InputDecorator(
        decoration: const InputDecoration(labelText: 'Product', prefixIcon: Icon(Icons.inventory_2_rounded), border: OutlineInputBorder()),
        child: Row(children: [
      ...
      builder: (context) => StatefulBuilder(builder: (context, setInner) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            TextField(decoration: const InputDecoration(hintText: 'Search by name or SKU', prefixIcon: Icon(Icons.search_rounded)), onChanged: (v) async {
              final q = v.trim();
            ...
            const SizedBox(height: 8),
            Flexible(child: results.isEmpty ? const Center(child: Text('No products')) : ListView.builder(shrinkWrap: true, itemCount: results.length, itemBuilder: (context, i) {
              final it = results[i];
              return RadioListTile<int>(value: it.productId, groupValue: selectedId, onChanged: (v) => setInner(() => selectedId = v), title: Text(it.name), subtitle: Text([(it.sku ?? '').isNotEmpty ? 'SKU: ${it.sku}' : null, 'Stock: ${it.stock.toStringAsFixed(2)}'].whereType<String>().join(' · ')));
            }))
          ]),
        ),
        actions: [TextButton(onPressed: () => Navigator.pop(context, null), child: const Text('Cancel')), FilledButton(onPressed: () {
          final it = results.firstWhere((e) => e.productId == selectedId, orElse: () => InventoryListItem(productId: -1, name: '', sku: null, categoryName: null, brandName: null, unitSymbol: null, reorderLevel: 0, stock: 0, isLowStock: false, price: null));
              ...
              children: [
                TextField(
                  ...
                  onChanged: (v) async {
                    final q = v.trim();
                    try {
                      final list = await repo.getSuppliers(search: q.isEmpty ? null : q);
                      setInner(() => results = list);
                    } catch (_) {}
                  },
                ),
