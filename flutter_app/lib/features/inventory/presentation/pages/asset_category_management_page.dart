...
class AssetCategoryManagementPage extends ConsumerStatefulWidget {
  const AssetCategoryManagementPage({super.key});

  @override
  ConsumerState<AssetCategoryManagementPage> createState() =>
      _AssetCategoryManagementPageState();
}

class _AssetCategoryManagementPageState
    extends ConsumerState<AssetCategoryManagementPage> {
  bool _loading = true;
  Object? _error;
  String _query = '';
  List<AssetCategoryDto> _items = const [];
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final results = await Future.wait([
        inventoryRepo.getAssetCategories(),
        accountsRepo.getLedgerBalances(),
      ]);
      if (!mounted) return;
      setState(() {
        _items = results[0] as List<AssetCategoryDto>;
  ...
  List<AssetCategoryDto> get _filteredItems {
  ...
  Future<void> _openDialog([AssetCategoryDto? existing]) async {
    final updated = await showDialog<bool>(
      context: context,
      builder: (context) => _AssetCategoryDialog(
        existing: existing,
        ledgers: _ledgers,
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Asset Classes'),
        actions: [
          IconButton(
      ...
      body: SafeArea(
        child: _loading && _items.isEmpty
            ? const AppLoadingView(label: 'Loading asset classes')
                      ...
                      Padding(
                        ...
                        child: TextField(
                          decoration: const InputDecoration(
                            hintText: 'Search class or ledger',
                      ...
                      Expanded(
                        child: _filteredItems.isEmpty
                            ? const AppEmptyView(
                                ...
                                message:
                                    'Create asset classes to group capitalized items and drive fixed asset ledger posting.',
...
class _AssetCategoryDialog extends ConsumerStatefulWidget {
  const _AssetCategoryDialog({
    required this.ledgers,
    this.existing,
  });

  final List<LedgerBalanceDto> ledgers;
  final AssetCategoryDto? existing;

  @override
  ConsumerState<_AssetCategoryDialog> createState() =>
      _AssetCategoryDialogState();
}

class _AssetCategoryDialogState extends ConsumerState<_AssetCategoryDialog> {
  ...
  Future<void> _save() async {
    if (_nameController.text.trim().isEmpty) {
      _showMessage('Enter a class name.');
      return;
    }
    ...
    try {
      ...
      if (_isEdit) {
        await repo.updateAssetCategory(
        ...
        await repo.createAssetCategory(
          name: _nameController.text.trim(),
          description: _descriptionController.text,
          ledgerAccountId: _selectedLedger?.accountId,
        );
  ...
  Future<void> _delete() async {
    ...
    try {
      await ref
          ...
          .deleteAssetCategory(existing.categoryId);
      if (!mounted) return;
      Navigator.of(context).pop(true);
    } catch (e) {
      _showMessage(ErrorHandler.message(e));
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: Text(_isEdit ? 'Edit Asset Class' : 'New Asset Class'),
      content: SizedBox(
        width: 520,
                ...
                onTap: _saving
                    ...
                    : () async {
                        final picked =
                          ...
                          context: context,
                          title: 'Select Fixed Asset Ledger',
                      ...
                      Expanded(
                        child: Text(
                          _selectedLedger == null
                              ? 'Use default fixed asset ledger'
                              : [
