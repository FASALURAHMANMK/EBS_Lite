...
Flutter Help & Support now routes to a live operational page.
Flutter release builds now reject localhost `API_BASE_URL` values.
Launch-critical operator assets are now present in repo.
The web shell remains a limited secondary surface and should not be sold as full parity with Flutter.
...
Seeded finance/UAT data is now reproducible through the demo reset command.
Exact Remaining Gaps
...
/inventory/barcode is implemented on the backend and remains intentionally unexposed in Flutter for Phase 1.
...
Current decision: NO-GO until the manual release-candidate UAT sign-off is closed.
