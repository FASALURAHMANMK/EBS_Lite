-- +goose Up
-- Source: Docs & Schema/PostgrSQL.sql
-- Initializes the full ERP schema for a brand-new database.
...
DROP POLICY IF EXISTS workflow_templates_rls ON workflow_templates;
...
DROP POLICY IF EXISTS workflow_states_rls ON workflow_states;
...
DROP POLICY IF EXISTS workflow_approvals_rls ON workflow_approvals;
...
CREATE TABLE IF NOT EXISTS cash_register_tally (
...
DO $$
BEGIN
	IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_salary_components_payroll') THEN
...
CREATE INDEX IF NOT EXISTS idx_locations_active ON locations(company_id, is_active);

-- Users and Authentication
CREATE INDEX IF NOT EXISTS idx_users_company_location ON users(company_id, location_id);
CREATE INDEX IF NOT EXISTS idx_users_email ON users(email);
CREATE INDEX IF NOT EXISTS idx_users_username ON users(username);
CREATE INDEX IF NOT EXISTS idx_device_sessions_user_id ON device_sessions(user_id);
CREATE INDEX IF NOT EXISTS idx_device_sessions_active ON device_sessions(user_id, is_active);
CREATE UNIQUE INDEX IF NOT EXISTS ux_password_reset_tokens_token ON password_reset_tokens(token);
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_product_barcodes_barcode ON product_barcodes(barcode);
CREATE UNIQUE INDEX IF NOT EXISTS ux_product_barcodes_product_id_barcode ON product_barcodes(product_id, barcode);
CREATE UNIQUE INDEX IF NOT EXISTS ux_product_barcodes_primary ON product_barcodes(product_id) WHERE is_primary;
...
CREATE INDEX IF NOT EXISTS idx_product_attribute_values_product_attribute ON product_attribute_values(product_id, attribute_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_sales_location_idempotency ON sales(location_id, idempotency_key) WHERE idempotency_key IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_sales_created_by ON sales(created_by);
CREATE INDEX IF NOT EXISTS idx_sales_location_created_at ON sales(location_id, created_at);
...
CREATE INDEX IF NOT EXISTS idx_purchases_location_created_at ON purchases(location_id, created_at);
...
CREATE INDEX IF NOT EXISTS idx_goods_receipts_location_received_date ON goods_receipts(location_id, received_date);

-- Returns
CREATE INDEX IF NOT EXISTS idx_sale_returns_sale ON sale_returns(sale_id);
CREATE INDEX IF NOT EXISTS idx_sale_returns_date ON sale_returns(return_date);
CREATE INDEX IF NOT EXISTS idx_sale_returns_location_created_at ON sale_returns(location_id, created_at);
...
CREATE INDEX IF NOT EXISTS idx_purchase_returns_location_created_at ON purchase_returns(location_id, created_at);
...
CREATE INDEX IF NOT EXISTS idx_customer_credit_transactions_company_customer_created_at ON customer_credit_transactions(company_id, customer_id, created_at);

-- Financial
CREATE UNIQUE INDEX IF NOT EXISTS ux_taxes_company_name ON taxes(company_id, name);
CREATE UNIQUE INDEX IF NOT EXISTS ux_payment_methods_company_name ON payment_methods(company_id, name);
CREATE INDEX IF NOT EXISTS idx_payments_supplier ON payments(supplier_id);
CREATE INDEX IF NOT EXISTS idx_payments_location_created_at ON payments(location_id, created_at);
...
CREATE INDEX IF NOT EXISTS idx_collections_location_created_at ON collections(location_id, created_at);
...
CREATE INDEX IF NOT EXISTS idx_cash_register_location_date ON cash_register(location_id, date);
CREATE INDEX IF NOT EXISTS idx_cash_register_tally_location_recorded_at ON cash_register_tally(location_id, recorded_at);
CREATE INDEX IF NOT EXISTS idx_vouchers_company_date ON vouchers(company_id, date);
CREATE INDEX IF NOT EXISTS idx_ledger_entries_account ON ledger_entries(account_id);
CREATE INDEX IF NOT EXISTS idx_ledger_entries_company_account ON ledger_entries(company_id, account_id);
...
CREATE INDEX IF NOT EXISTS idx_attendance_employee_check_in ON attendance(employee_id, check_in);
CREATE INDEX IF NOT EXISTS idx_payroll_employee ON payroll(employee_id);
CREATE INDEX IF NOT EXISTS idx_payroll_period ON payroll(pay_period_start, pay_period_end);
CREATE INDEX IF NOT EXISTS idx_salary_components_payroll ON salary_components(payroll_id);
CREATE INDEX IF NOT EXISTS idx_payroll_advances_payroll ON payroll_advances(payroll_id);
...
CREATE INDEX IF NOT EXISTS idx_invoice_formats_company_location ON invoice_formats(company_id, location_id);
CREATE INDEX IF NOT EXISTS idx_numbering_sequences_company_location ON numbering_sequences(company_id, location_id);
...
('ko', 'Korean'),
('it', 'Italian'),
('nl', 'Dutch') ON CONFLICT DO NOTHING;

-- Default Currency
INSERT INTO currencies (code, name, symbol, is_base_currency) VALUES 
('USD', 'US Dollar', '$', true) ON CONFLICT DO NOTHING;
...
('Accountant', 'Accounting operations', true) ON CONFLICT DO NOTHING;
...
('Centimeters', 'cm'),
('Boxes', 'box'),
('Dozens', 'doz') ON CONFLICT DO NOTHING;
...
('Debit Card', 'CARD'),
('Bank Transfer', 'BANK'),
('Check', 'BANK') ON CONFLICT DO NOTHING;
...
DROP POLICY IF EXISTS locations_rls ON locations;
...
DROP POLICY IF EXISTS purchases_rls ON purchases;
...
DROP POLICY IF EXISTS sale_returns_rls ON sale_returns;
...
DROP POLICY IF EXISTS purchase_returns_rls ON purchase_returns;
...
DROP POLICY IF EXISTS goods_receipts_rls ON goods_receipts;
...
DROP POLICY IF EXISTS stock_transfers_rls ON stock_transfers;
...
DROP POLICY IF EXISTS collections_rls ON collections;
...
ALTER TABLE IF EXISTS stock_transfers
	ADD COLUMN IF NOT EXISTS workflow_state_id INT REFERENCES workflow_states(state_id);
...
('ASSIGN_PERMISSIONS', 'Assign permissions to roles', 'roles', 'assign') ON CONFLICT DO NOTHING;
...
WHERE name IN ('VIEW_DASHBOARD', 'VIEW_USERS', 'CREATE_USERS', 'UPDATE_USERS', 'DELETE_USERS', 
               ...
               'UPDATE_LOCATIONS', 'DELETE_LOCATIONS', 'VIEW_ROLES') ON CONFLICT DO NOTHING;
...
WHERE name IN ('VIEW_DASHBOARD', 'VIEW_USERS', 'CREATE_USERS', 'UPDATE_USERS', 
               'VIEW_LOCATIONS', 'UPDATE_LOCATIONS') ON CONFLICT DO NOTHING;
...
WHERE name LIKE '%PRODUCTS%' OR name LIKE '%INVENTORY%' ON CONFLICT DO NOTHING;
...
('APPROVE_TRANSFERS', 'Approve and complete stock transfers', 'inventory', 'approve') ON CONFLICT DO NOTHING;
...
WHERE name IN ('VIEW_PRODUCTS', 'CREATE_PRODUCTS', 'UPDATE_PRODUCTS', 'DELETE_PRODUCTS',
               'VIEW_INVENTORY', 'ADJUST_STOCK', 'CREATE_TRANSFERS', 'APPROVE_TRANSFERS') ON CONFLICT DO NOTHING;

-- Assign basic permissions to Manager role (role_id = 3)
INSERT INTO role_permissions (role_id, permission_id)
SELECT 3, permission_id FROM permissions 
WHERE name IN ('VIEW_PRODUCTS', 'CREATE_PRODUCTS', 'UPDATE_PRODUCTS',
               'VIEW_INVENTORY', 'ADJUST_STOCK', 'CREATE_TRANSFERS') ON CONFLICT DO NOTHING;

-- Assign view permissions to Sales role (role_id = 4)
INSERT INTO role_permissions (role_id, permission_id)
SELECT 4, permission_id FROM permissions 
WHERE name IN ('VIEW_PRODUCTS', 'VIEW_INVENTORY') ON CONFLICT DO NOTHING;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_loyalty_points_positive') THEN
        ALTER TABLE loyalty_programs ADD CONSTRAINT chk_loyalty_points_positive CHECK (points >= 0);
    END IF;
END;
$$;
-- +goose StatementEnd
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_loyalty_totals_positive') THEN
        ALTER TABLE loyalty_programs ADD CONSTRAINT chk_loyalty_totals_positive CHECK (total_earned >= 0 AND total_redeemed >= 0);
    END IF;
END;
$$;
-- +goose StatementEnd
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_redemption_points_positive') THEN
        ALTER TABLE loyalty_redemptions ADD CONSTRAINT chk_redemption_points_positive CHECK (points_used > 0 AND value_redeemed > 0);
    END IF;
END;
$$;
-- +goose StatementEnd
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_promotion_dates') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_return_amount_positive') THEN
        ALTER TABLE sale_returns ADD CONSTRAINT chk_return_amount_positive CHECK (total_amount >= 0);
    END IF;
END;
$$;
-- +goose StatementEnd
...
DO $$
BEGIN
	IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_sad_location') THEN
...
DO $$
BEGIN
	IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_sadi_product') THEN
...
WHERE NOT EXISTS (SELECT 1 FROM permissions WHERE name = 'VIEW_INVENTORY') ON CONFLICT DO NOTHING;

-- ADJUST_STOCK
INSERT INTO permissions (name, description, module, action)
SELECT 'ADJUST_STOCK', 'Create stock adjustments', 'INVENTORY', 'ADJUST'
WHERE NOT EXISTS (SELECT 1 FROM permissions WHERE name = 'ADJUST_STOCK') ON CONFLICT DO NOTHING;
...
BEGIN
    ...
    VALUES (TG_OP, 'users', COALESCE(NEW.user_id, OLD.user_id), to_jsonb(OLD), to_jsonb(NEW), CURRENT_TIMESTAMP) ON CONFLICT DO NOTHING;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
-- +goose StatementEnd
...
BEGIN
    INSERT INTO audit_log(action, table_name, record_id, old_value, new_value, timestamp)
    VALUES (TG_OP, 'products', COALESCE(NEW.product_id, OLD.product_id), to_jsonb(OLD), to_jsonb(NEW), CURRENT_TIMESTAMP) ON CONFLICT DO NOTHING;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
-- +goose StatementEnd
...
BEGIN
    INSERT INTO audit_log(action, table_name, record_id, old_value, new_value, timestamp)
    VALUES (TG_OP, 'sales', COALESCE(NEW.sale_id, OLD.sale_id), to_jsonb(OLD), to_jsonb(NEW), CURRENT_TIMESTAMP) ON CONFLICT DO NOTHING;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
-- +goose StatementEnd
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_products_tax') THEN
